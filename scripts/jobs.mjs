 ...
 *   node scripts/jobs.mjs day_close|alerts_eval|anchor_reminder|daily_summary|gmail_scan|drive_intake_scan|calendar_sync|weekly_report|pnl_draft|pnl_final|accountant_pack|deal_decay|db_backup|greeninvoice_import|wise_import [YYYY-MM-DD]
 *
...
if (!name) { console.error('שימוש: node scripts/jobs.mjs <day_close|alerts_eval|anchor_reminder|daily_summary|gmail_scan|drive_intake_scan|calendar_sync|weekly_report|pnl_draft|pnl_final|accountant_pack|deal_decay|db_backup|greeninvoice_import|wise_import> [YYYY-MM-DD]'); process.exit(1) }
